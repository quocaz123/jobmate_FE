...
import { MapPin, DollarSign, Plus, Trash2, Activity, X } from "lucide-react";
...
const JOBS_DATA = [
  ...
  },
  {
    ...
    type: "Freelance",
    description: "Thiết kế poster sáng tạo cho các sự kiện âm nhạc và hội thảo.",
  },
  {
    id: 3,
...
export default function JobRequest() {
  ...
  const suggestedJobs = useMemo(() => {
    ...
    return JOBS_DATA.map((j) => ({ ...j, match: computeMatchPercent(req, j) }));
  }, [skills, type, location]);
  ...
  return (
    <div className="w-full max-w-7xl mx-auto space-y-6">
      <h1 className="text-2xl font-semibold text-gray-800 mb-6">Tạo yêu cầu tìm việc</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <form onSubmit={handleCreate} className="lg:col-span-2 space-y-6">
          ...
          <section className="bg-white rounded-2xl border border-indigo-100 shadow-sm p-6">
            <h2 className="text-lg font-medium text-indigo-700 mb-3">Thông tin cơ bản</h2>
            <input
              value={title}
            ...
            <div className="grid grid-cols-2 gap-3 mt-3">
              <select value={location} onChange={(e) => setLocation(e.target.value)} className="border border-indigo-200 rounded-md px-3 py-2">
                <option>Hà Nội</option>
                <option>TP.HCM</option>
                <option>Đà Nẵng</option>
                <option>Remote</option>
              </select>
              <select value={type} onChange={(e) => setType(e.target.value)} className="border border-indigo-200 rounded-md px-3 py-2">
          ...
          <section className="bg-white rounded-2xl border border-indigo-100 shadow-sm p-6">
            ...
            <div className="flex gap-2">
              ...
              />
              <button type="button" onClick={addSkill} className="p-2 bg-indigo-600 text-white rounded-md">
                <Plus size={14} />
              </button>
            </div>
            <div className="flex flex-wrap gap-2 mt-3">
              {skills.map((s, i) => (
                <div key={i} className="px-3 py-1 rounded-full bg-indigo-50 text-indigo-700 border border-indigo-100 flex items-center gap-2">
                  {s}
          ...
          <div className="flex items-center gap-4">
            <button type="submit" disabled={loading} className="px-6 py-3 rounded-full text-white bg-gradient-to-r from-indigo-600 to-blue-500 shadow">
              {loading ? "Đang tạo..." : "Tạo yêu cầu"}
            </button>
            <button type="button" onClick={() => setPreviewOpen(true)} className="px-4 py-2 rounded-full border text-indigo-700 hover:bg-indigo-50">
              Xem trước
            </button>
          </div>
        </form>
  ...
  <aside className="space-y-4 lg:sticky lg:top-6">
          <div className="bg-white rounded-2xl border border-indigo-100 shadow-sm p-4">
            ...
            <div className="space-y-3">
              {suggestedJobs.map((j) => (
                <div key={j.id} className="p-3 border border-indigo-50 rounded-lg hover:shadow transition bg-white">
                  ...
                  <div className="mt-2 flex justify-between items-center">
                    ...
                    <button onClick={() => setSelectedJob(j)} className="px-3 py-1 rounded-md bg-indigo-600 text-white text-xs">
                      Xem chi tiết
                    </button>
                  </div>
                </div>
      ...
      {selectedJob && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-2xl w-full max-w-md relative">
            <button className="absolute top-2 right-2 text-gray-400 hover:text-gray-600" onClick={() => setSelectedJob(null)}>
              <X size={18} />
            </button>
            ...
            <p className="text-sm text-gray-500">💰 {selectedJob.salary} — 📍 {selectedJob.location}</p>
          </div>
        </div>
      )}
      ...
      {previewOpen && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-2xl w-full max-w-md relative">
            <button className="absolute top-2 right-2 text-gray-400 hover:text-gray-600" onClick={() => setPreviewOpen(false)}>
              <X size={18} />
            </button>
            <h3 className="text-lg font-semibold text-indigo-700 mb-2">Xem trước yêu cầu của bạn</h3>
            ...
            <p className="text-sm text-gray-700 mt-2"><b>Kỹ năng:</b> {skills.join(", ")}</p>
          </div>
        </div>
      )}
    </div>
